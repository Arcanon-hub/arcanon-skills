...
- **Read** — read `.arcanon.toml` and source files to understand current config and code patterns
- **Grep** — search source files for import statements and function calls
- **Write/Edit** — create or update `.arcanon.toml`
- **Bash** — run `arcanon --dry-run -vv` to verify patterns work
...
2. **Determine the import gate** — the import/require string that identifies the library in the file
